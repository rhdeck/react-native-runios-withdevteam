...
const developmentTeam = require("xcode-devteam-management");
const getBuildPath = function(configuration = "Debug", appName, isDevice) {
  return `build/Build/Products/${configuration}-${
    isDevice ? "iphoneos" : "iphonesimulator"
  }/${appName}.app`;
};
...
function runIOS(argv, config, args) {
  ...
  if (args.developmentTeam) {
    if (
      developmentTeam.getDevTeam() &&
      (args.developmentTeam === true || !args.developmentTeam.length)
    ) {
      console.log(
        "Using saved development team:",
        developmentTeam.getDevTeam()
      );
      devteam = developmentTeam.getDevTeam();
    } else if (args.developmentTeam.length != 10) {
      developmentTeam.getFromDevTeams().then(
        devteam => {
          if (!devteam) {
            console.log(
              "I did not get a development team I can use - aborting"
            );
            process.exit(1);
          }
...
function buildProject(
  ...
  return new Promise((resolve, reject) => {
    ...
    const buildProcess = child_process.spawn(
      "xcodebuild",
      xcodebuildArgs,
      getProcessOptions(launchPackager)
    );
    let buildOutput = "";
...
module.exports = {
  ...
  options: [
    ...
    },
    {
      ...
      description:
        "Explicitly set development team. Uses saved development team ID or interactive prompt if not specified"
    }
  ]
};
