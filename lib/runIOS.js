...
function runIOS(argv, config, args) {
  ...
  if(args.developmentTeam) {
    if(developmentTeam.getDevTeam() && (args.developmentTeam === true || !args.developmentTeam.length)) {
        console.log("Using saved development team:", developmentTeam.getDevTeam());
        devteam = developmentTeam.getDevTeam(); 
    } else if(args.developmentTeam.length != 10) {
      developmentTeam.getFromDevTeams().then((devteam)=>{
        if(!devteam) {
...
module.exports = {
  ...
  options: [{
    ...
    description: "Explicitly set development team. Uses saved development team ID or interactive prompt if not specified"
  }],
};
