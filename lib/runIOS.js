    ...
    if(args.developmentTeam === true) {
      if(homeinfo.devteam) {
        ...
        inquirer.prompt([{
          ...
          return runIOS(argv, config, args);          
        },(error)=>{});
      } else {
        inquirer.prompt([{
          name: "searchortype",
          ...
          if(answers.searchortype) {
            ...
            const command  = "find " + homedir + "/Documents | grep -m500 \"project\\.pbxproj\" | xargs -L1 -JABC grep \"evelopmentTeam\" \"ABC\" 2>/dev/null | sort | uniq";
        ...
        return false
        console.log("I have no saved development team, try providing one! Its the 10-digit number Apple assigned to your developer account or team")
        return false; 
      }
    } else if(args.developmentTeam.length != 10) {
      console.log("This is not a valid development team UUID (needs to be 10 characters):", args.developmentTeam);
      return false; 
    } else {
